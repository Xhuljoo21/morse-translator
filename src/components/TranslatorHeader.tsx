...
export function TranslatorHeader({ mode, onToggleMode }: TranslatorHeaderProps) {
  return (
    <div className="flex items-center justify-between mb-8">
      <div className="flex items-center gap-3">
        <BrainCircuit className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Morse Code Translator</h1>
      </div>
      <div className="flex items-center gap-3">
        <ThemeToggle />
        <button
        ...
        >
          {mode === 'text-to-morse' ? 'Switch to Morse → Text' : 'Switch to Text → Morse'}
        </button>
      </div>
    </div>
  );
